...
base_model = VGG16(weights="imagenet", include_top=False,
                                input_shape=(size, size, 3),backend=keras.backend,
                                          layers=keras.layers,
                                          models=keras.models,
                                          utils=keras.utils)  # 预训练的VGG16网络，替换掉顶部网络
...
top_model.add(Dense(512, activation='relu'))  # 全连接层，输入像素256
top_model.add(Dropout(0.5))  # Dropout概率0.5
top_model.add(Dense(256, activation='relu'))
top_model.add(Dropout(0.5))  # Dropout概率0.5
top_model.add(Dense(4))  # 输出层，二分类
...
xpath=r'.\data\train\fault_fold'
filelist = os.listdir(xpath) 
images=[]
labels=label

val_label = np.loadtxt(r'val.txt')
val_xpath=r'.\data\val\fault_fold'
...
checkpointer = ModelCheckpoint(filepath='fault_fold.h5', verbose=1, save_best_only=True)
